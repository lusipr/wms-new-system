...
export const Client = () => {
  ...
  const barcodeData = useMemo(() => {
    return (
      dataBarcode?.data.resource.produk ?? {
        id: "0",
        old_barcode_product: "",
        old_name_product: "",
  ...
  useEffect(() => {
    if (barcodeData?.id && barcodeData.id !== "0") {
      setIdProduct(barcodeData.id);
    }
  }, [barcodeData]);

  const tagColor = useMemo(() => {
    return (
      dataBarcode?.data.resource?.color_tags?.[0] ?? {
        id: "0",
        hexa_code_color: "",
        name_color: "",
        fixed_price_color: "0",
  ...
  const handleSubmit = (e: FormEvent, type: string) => {
    e.preventDefault();
    if (!idProduct) {
      toast.error("Product ID not found. Please scan barcode again.");
      return;
    }
    ...
    const body = {
      ...
      quality: type,
      category_id:
        type === "lolos" ? Number(selectedCategory?.id ?? null) : null,
      tag_color_id: tagColor?.id ? Number(tagColor.id) : null,
      deskripsi:
        type === "abnormal"
          ? metaData.abnormal
          : type === "damaged"
  ...
  useEffect(() => {
    if (
      ...
      (isSuccessBarcode && !dataBarcode?.status)
    ) {
      toast.error(
  ...
  useEffect(() => {
    if (!Number.isInteger(metaData.qty) || metaData.qty < 0) {
  ...
  return (
    <div className="flex flex-col items-start bg-gray-100 w-full relative px-4 gap-4 py-4">
      ...
      {loadingBarcode || isPendingSubmit || isPendingDouble ? (
        ...
        <div className="flex w-full bg-white rounded-md shadow items-center justify-center h-75">
