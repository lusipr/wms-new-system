...
export const Client = () => {
  ...
  const { codeDocumentId } = useParams();
      ...
      cell: ({ row }) => (
        <div className="flex gap-4 justify-center items-center">
          <TooltipProviderPage value="Check">
            <Button
              className="items-center px-0  border-gray-400 text-gray-700 hover:text-gray-700 hover:bg-gray-50 w-9"
              variant={"outline"}
              type="button"
              onClick={(e) => {
                e.preventDefault();
                const id = row.original.old_barcode_product;
                router.push(
                  `/inbound/check-product/manifest-inbound/${codeDocumentId}/check?q=${id}`
                );
              }}
            >
    ...
    <div className="flex flex-col items-start bg-gray-100 w-full relative px-4 gap-4 py-4">
      ...
      <div className="flex text-sm text-gray-500 py-8 rounded-md shadow bg-white w-full px-5">
        ...
        <div className="flex w-full">
          <div className="flex flex-col items-end w-1/5 border-r border-gray-400! pr-5 mr-5">
            <p className="text-sm font-medium">Status</p>
            <h3
              className={cn(
                "px-3 py-1 rounded-2xl font-light capitalize whitespace-nowrap border",
                dataMetaDetailMI?.status === "pending" &&
                  "bg-[#EFF8FF] text-[#FFAE00] border-[#F6FF7E]!",
                dataMetaDetailMI?.status === "done" &&
                  "bg-[#EFF8FF] text-blue-700 border-blue-200!",
                dataMetaDetailMI?.status !== "pending" &&
                  dataMetaDetailMI?.status !== "done" &&
          ...
          <div className="flex flex-col items-end w-2/5 border-r border-gray-400! pr-5 mr-5">
            <p className="text-sm font-medium">Merged Data</p>
            <h3 className="px-3 py-1 rounded-2xl border bg-[#EFF8FF] text-[#22C55E] border-[#79FF7E]! font-light capitalize">
              {dataMetaDetailMI?.code_document}
            </h3>
          </div>
          <div className="flex flex-col items-end w-1/5 border-r border-gray-400! pr-5 mr-5">
            <p className="text-sm font-medium">Total</p>
            <h3 className="px-3 py-1 rounded-2xl border bg-[#EFF8FF] text-blue-700 border-blue-200! font-light capitalize">
          ...
          <div className="flex flex-col items-end w-auto">
            <Dialog open={cEBarcode} onOpenChange={setCEBarcode}>
              <DialogTrigger asChild>
                <button className="text-sm gap-3 font-medium flex items-center bg-[#0B91FF] hover:bg-sky-400 rounded pr-2 pl-3  text-white focus-visible:outline-none focus-visible:ring-0 group border border-sky-400">
                  ...
                  <div className="flex items-center justify-center size-6 group-hover:bg-sky-200 hover:bg-sky-200 rounded-full border border-sky-700">
      ...
      <div className="flex w-full bg-white rounded-md overflow-hidden shadow px-5 py-3 gap-1 flex-col">
            ...
            <div className="flex gap-2 items-center w-full flex-auto">
              ...
              <div className="flex items-center justify-end w-full gap-3">
                ...
                <Button
                  ...
                  className="bg-[#0B91FF] hover:bg-sky-400 text-black"
                >
                  <Link
