...
export const Client = () => {
  ...
  useEffect(() => {
    if (data && isSuccess) {
      setPagination(data?.data.resource.products);
    }
  }, [data, isSuccess]);
  ...
  const columnSales: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      header: "Category",
      cell: ({ row }) => (
        <div className="max-w-125 break-all">{row.original.category_name}</div>
      ),
    },
    {
      ...
      header: "Qty",
      cell: ({ row }) => (
        <div className="max-w-125 break-all">{row.original.quantity}</div>
      ),
    },
    {
        ...
        <div className="flex gap-4 justify-center items-center">
          <TooltipProviderPage value={"Add Product"}>
            <Button
              ...
              onClick={(e) => {
                e.preventDefault();
                handleAddProduct(row.original.product_barcode);
              }}
              type="button"
